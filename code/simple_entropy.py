...
def xlogx(x, base=None):
    if x == 0: return 0
    if base == 2: return x*np.log2(x)
    if base is None: return x*np.log(x)
...
if __name__ == '__main__':
    ...
    ax1.plot(x1, y1, c='black', ls='-', lw='2')

    ax1.set_xlim([0,1])
    ax1.set_ylim([0,1])
    ax1.set_xlabel(r'$p$')
    ax1.set_ylabel(r'$\mathrm{\mathbb{H}}$ (bits)')
    ax1.axvline(0.5, c='black', lw=2, alpha=0.2)
    ...
    y2 = 1-np.log(x2)
    ax2.plot(x2, y2, c='black', ls='-', lw='2')
    ...
    ax3.plot(x3, y3, c='black', ls='-', lw='2')
...
#    ax3.set_ylim([y3[-1], y3[1]])
    ...
    ax3.axvline(1, c='black', lw=2, alpha=0.2)
