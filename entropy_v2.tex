...
The solid line marked by circles represents the part of the path entropy related to sojourn-associated uncertainties (Eq.~\eqref{eqn:H_continuous}), which as a weighted average of one-pool entropies, it decreases similarly as the entropy of an exponential distribution with increasing rate parameter $\lambda$ (Fig.~\ref{fig:simple_entropy}, panel (b)).
...
% 	results in the same transfer function, but a different entropy rate per unit time, \ie, $\theta(\widetilde{M}) \approx 1.903\,\nats\,\yr^{-1}$.
	...
	\caption{
		Local maximizations of $\theta$ over a grid on a subspace of the parameter space. For better visibility we chose randomly $1000$ grid points for the plot. Blue dots show local maxima found during the global maximization procedure starting on the grid. 
		...
		(c) Paths of entropy rate during the local maximizations on the grid.
