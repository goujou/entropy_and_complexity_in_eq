...
For many modeling applications, it is of interest to quantify the complexity of the system of differential equations used to represent natural phenomena \citep{Burnham2002, Hoege2018}. In principle, we are interested in selecting models that are parsimonious; i.e. with the least degree of complexity for explaining certain patterns in nature \citep{Golan2022}. 
The concept of entropy has been commonly used to characterize complexity or information content. Classical entropy measures for dynamical systems characterize the rate of increase in dynamical complexity as the system evolves over time \citep{Jost2005}. These metrics have been used extensively to characterize chaotic behavior in complex nonlinear systems \citep{Fan2021}, but as we will see later, they give trivial results for a large range of models used in geosciences and biology. 
...
Our information theoretical approach differs from the thermodynamic approach to entropy, which has been developed by other authors studying energy transfers and reversibility in thermodynamic systems \citep{Aoki1988, Haddad2010, Haddad2013, Haddad2019}
...
The nonnegative vector $\vec{x}(t)$ describes the amount of material in the different compartments at time $t$, the nonnegative vector $\vec{u} \in\R^d$ is the vector of external inputs to the compartments, and the compartmental matrix $\tens{B}\in\R^{d\times d}$ describes the flux rates between the compartments and out of the system.
...
This implies that the outflux vector from the system is given by $\vec{r} = \vec{z}^T\,\tens{B}$ and, since the system is in equilibrium, $\|\vec{r}\| = \|\vec{u}\|$, where $\vnorms{\cdot}$ denotes the sum of absolute values of a vector ($l_1$-norm). 
...
The superscript $T$ denotes the transpose of the vector/matrix. 
% and $\vnorms{\vec{u}}=\sum_{i=1}^d |u_i|$ denotes the $l_1$-norm of the vector $\vec{u}$.
...
The sequence $Y_1,Y_2,\ldots,Y_{\mathcal{N}-1}\in\mathcal{S}$ represents the successively visited compartments  with the associated sojourn times $T_1,T_2,\ldots,T_{\mathcal{N}-1}$, the random variable
\begin{equation}
...
\begin{equation}
  \label{eqn:N_i}
  \E\left[N_i\right] = \suml_{j=1}^d M_{ij}\,\beta_j = (\tens{M}\,\vec{\beta})_i = \left[(\tens{I}-\tens{P}_{\tens{B}})^{-1}\,\vec{\beta}\right]_i 
...
$(Z_n)_{n\geq1}=(\widetilde{Y}_n,\widetilde{T}_n)_{n\geq1}$ on the space $(\widetilde{S}\times\R_+)$ defined by the transition probabilities $\widetilde{P}_{ij}(t) = \P(\widetilde{Y}_{n+1}=i, \widetilde{T}_{n+1}\leq t\,|\,\widetilde{Y}_n=j)$ given by
\begin{equation}
  \widetilde{P}_{ij}(t) = 
  \begin{cases}
     0, & i=j,\\
     ...
     \beta_i\,(1-e^{-\lambda_i\,t}), & i\leq d+1,\,j=d+1,
  \end{cases}
\end{equation}
and initital (stationary) distribution
\begin{equation}
  ...
  \end{cases}
\end{equation}
This process describes the infinite journey, \ie\ the sequence of visited compartments with the associated sojourn times, of a single particle through the system with immediate jumps back into the system when reaching the environmental state.
...
\begin{proof}
 ...
 To that end, we define $\pi_j:=\lim_{t\to\infty} \pi_j(t)$, and we prove $\P(\widetilde{Y}_2=i,\widetilde{T}_2\leq t) = \pi_i(t)$. Stationarity follows then by induction. Let $i=d+1$.
 Then,
\begin{equation}
  \begin{aligned}
    ...
    &= \suml_{j=1}^d \frac{B_{ij}}{\lambda_j}\,\frac{\E\left[N_j\right]}{\E\left[\mathcal{N}\right]}.
  \end{aligned}
\end{equation}
By Eq.~\eqref{eqn:N_i}, $\vec{r}=\vec{z}^T\,\vec{x}^\ast$, and $\|\vec{r}\|=\|\vec{u}\|$, we get
\begin{equation}
    \P(\widetilde{Y}_2=i,\widetilde{T}_2\leq t)
    = \frac{1}{\E\left[\mathcal{N}\right]}\suml_{j=1}^d \frac{B_{ij}}{\lambda_j}\,\frac{\lambda_j\,x^\ast_j}{\|\vec{u}\|}
    = \frac{1}{\E\left[\mathcal{N}\right]}\,\frac{\vec{z}^T\,\vec{x}^\ast}{\|\vec{u}\|} = \pi_{d+1}(t).
\end{equation}
Now let $i\leq d$.
Then
\begin{equation}
 \begin{aligned}
  ...
  &= \suml_{\substack{j=1, \\ j\neq i}}^d \frac{B_{ij}}{\lambda_j}\,(1-e^{-\lambda_i\,t})\,\frac{\E\left[N_j\right]}{\E\left[\mathcal{N}\right]} + \beta_i\,(1-e^{-\lambda_i\,t})\,\frac{1}{\E\left[\mathcal{N}\right]}\\
  &= \frac{1}{\E\left[\mathcal{N}\right]} \left[\suml_{\substack{i=1,\\i\neq j}}^d \frac{B_{ij}\,x^\ast_j}{\|\vec{u}\|} + \beta_i\right]\,(1-e^{-\lambda_i\,t})\\
  &= \frac{1}{\E\left[\mathcal{N}\right]} \left[\frac{(\tens{B}\,\vec{x}^\ast)_i}{\|\vec{u}\|} - \frac{B_{ii}\,x^\ast_i}{\|\vec{u}\|} + \beta_i\right]\,(1-e^{-\lambda_i\,t})\\
  &= \frac{1}{\E\left[\mathcal{N}\right]}\left[-\frac{u_i}{\|\vec{u}\|} + \frac{\lambda_i\,x^\ast_i}{\|\vec{u}\|} + \beta_i\right]\,(1-e^{\lambda_i\,t})\\
  &= \frac{1}{\E\left[\mathcal{N}\right]}\,\E\left[N_i\right]\,(1-e^{-\lambda_i\,t})\\
  &= \pi_i(t).
 \end{aligned}
\end{equation} 

Step 2. Since $Z$ is stationary, by \citet[Theorem~4.2.1]{Cover2006}, the entropy rate of $Z$ is given by
\begin{equation}
...
\begin{equation}
  \suml_{j=1}^d\pi_j\,\lambda_j^{-1} = \frac{1}{\E\left[\mathcal{N}\right]}\,\suml_{j=1}^d \frac{x^\ast_j}{\|\vec{u}\|} = \frac{\E\left[\TT\right]}{\E\left[\mathcal{N}\right]}.
\end{equation}
Hence,
\begin{equation}
...
In the next two examples we identify compartmental models with maxmimum entropy under some restrictions.
...
Those two models are well understood in equilibrium, hence they can serve as a means to better understand properties of the newly introduced entropy metrics.
...
While in ecosystem models the former process is almost exclusively based \citep{zaehle2014evaluation} on the Farquhar model \citep{farquhar1980biochemical}, the latter one is represented by a great variety models with different structure \citep{friedlingstein2006climate}, which leads to large variations in  the prediction of future land carbon uptake \citep{friedlingstein2006climate,friedlingstein2014uncertainties}.
...
While the photosynthetic apparatus operates on tim scales in the order of split seconds to minutes, soil carbon cycling rates are in the order of decades to millenia.
...
At the end of this section we apply MaxEnt to solve an equifinality problem in model selection as an example for how to tackle this problem arising from, for instance, tracer experiments.
...
\begin{table}[htbp]
   ...
   \begin{tabular}{@{} ccccccr @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      \midrule
       \input{figs/Compartments/onepool.tex}  & $-\lambda x +1$ & $0.5 \ (1 - \log \lambda)$ & 2.00 & $\lambda (1 - \log \lambda)$ & $1/ \lambda$ & $1 - \log$ \\
       \input{figs/Compartments/twoseries.tex}    & $\left( \begin{matrix} -1 & 0 \\ 1 & -1  \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 0 \end{matrix} \right)$ & 0.67 & 3.00 & 1.00 & 2.00 & 2.00  \\
         \input{figs/Compartments/twoparallel.tex}  & $\left( \begin{matrix} -1 & 0 \\ 0 & -1  \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 1 \end{matrix} \right)$ & 0.85 & 2.00 & 1.69 & 1.00 & 1.69 \\
         \input{figs/Compartments/twofeedback1.tex}  & $\left( \begin{matrix} -1 & 1/2 \\ 1 & -1  \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 0 \end{matrix} \right)$ & 1.08 & 5.00 & 1.35 & \underline{4.00} & \underline{5.39} \\
         \input{figs/Compartments/twofeedback2.tex}   & $\left( \begin{matrix} -1 & 1/2 \\ 1/2 & -1  \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 1 \end{matrix} \right)$ & \underline{1.36} & 3.00 & 2.04 & 2.00 & 4.08 \\
         \input{figs/Compartments/threeseries.tex}   & $\left( \begin{matrix} -1 & 0 & 0 \\ 1 & -1 & 0 \\ 0 & 1 & -1 \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 0 \\ 0 \end{matrix} \right)$ & 0.75 & 4.00 & 1.00 & 3.00 & 3.00 \\
         \input{figs/Compartments/threeparallel.tex}   & $\left( \begin{matrix} -1 & 0 & 0 \\ 0 & -1 & 0 \\ 0 & 0 & -1 \end{matrix} \right) x + \left( \begin{matrix} 1 \\ 1 \\ 1 \end{matrix} \right)$ & 1.05 & 2.00 & \underline{2.10} & 1.00 & 2.10 \\
      \bottomrule
   \end{tabular}
...
\subsection{Model identification via Maxent}
  ...
  It shows how MaxEnt can help take a decision which model to use if not all parameters can be uniquely determined from the transfer function $\widehat{\tens{\Psi}}$.
	...
	\end{equation}
	Maximizing the entropy rate per unit time here leads to a feasible optimization problem, whereas maximization of the path entropy by slowing down the model and indefiniteley increasing its mean transit time and with it its path entropy would lead to an unbounded optimization problem.
	...
	\end{equation}
	and is not guranteed to be convex in general.
	...
	Furthermore, local optimizations with starting points on the grid lead only to small improvements. A good choice of starting point on the grid is crucial to find a good global maxmimum candidate (c).
	Finally, the global maximum candidate for the entropy rate per unit time does not maximize the path entropy (d).
	
		
% 	The nonidentifiability of the model from $\widehat{\tens{\Psi}}$ alone is underlined by the fact hat the easy-to-guess) model $\widetilde{M}=M(\vec{u},\widetilde{\tens{B}})\in\mathcal{M}$ with
% 	\begin{equation}
% 		\widetilde{\tens{B}} =
% 		\begin{pmatrix}
% 			-2 & 2 \\ 1 & -3
...
% 		\,\yr^{-1}
% 	\end{equation}
% 	results in the same transfer function, but a different entropy rate per unit time, \ie, $\theta(\widetilde{M}) \approx 1.903\,\nats\,\yr^{-1}$.
...
This example is only aimed to give a first impression of how the maximum entropy principle can be used in combination with entropy rates or path entropy in similar situations.
Practical examples usually have a high level of complexity such that existence and uniqueness of a maximum entropy model, which would have to be studied on a case-by-case basis.
